...
def create_polygon_image(polygon: Polygon, color_mode: str, color=None, create_mask=True) -> Ndimg:
    ...
    if color is not None:
        ...
        cv2.fillPoly(img.ndarray, points, color)
    if create_mask:
        img_boolean_mask = create_polygon_image(polygon, 'L', 255, create_mask=False).ndarray
    else:
        img_boolean_mask = None
    return Ndimg(img.ndarray, color_mode, img_boolean_mask)
...
def draw_source_on_target_polygon(target: Ndimg, target_mask_polygon: Polygon, source: Ndimg) -> None:
    target_region = get_polygon_bbox_image_view(target.ndarray, target_mask_polygon)
    ...
    cv2.subtract(target_region, target_region, target_region, mask=source_.bool_mask_ndarray)
    # imgshow(target_region, 'Target(canvas) after substract')
    cv2.add(source_.ndarray, target_region, target_region, mask=source_.bool_mask_ndarray)
