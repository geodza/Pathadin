if __name__ == '__main__':
    ...
    labels_names = labels_loader.load_names()
    images_names = images_loader.load_names()
    from common.itertools_utils import peek
    from common.shuffle_utils import shuffle_lists
    labels_names, images_names = shuffle_lists(labels_names, images_names)
    labels_loader.names = labels_names
    images_loader.names = images_names

    # labels = labels_loader.stack_ndarrays()
    # images = images_loader.stack_ndarrays()
    labels = labels_loader.load_ndarrays()
    images = images_loader.load_ndarrays()
    ...
    from common_keras.image_data_generator2 import ImageDataGenerator2

    augment_kwargs = dict()
    data_generator = ImageDataGenerator2(**augment_kwargs)
    ...
    image_names_train, image_names_test, label_names_train, label_names_test = train_test_split(images_names, labels_names, test_size=0.25)
    images_train=NdarrayLoaderFactory.from_names(str(patches_path), image_names_train, convert_image).load_ndarrays()
    images_test=NdarrayLoaderFactory.from_names(str(patches_path), image_names_test, convert_image).load_ndarrays()
    labels_train=NdarrayLoaderFactory.from_names(str(patches_path), label_names_train, convert_label).load_ndarrays()
    labels_test=NdarrayLoaderFactory.from_names(str(patches_path), label_names_test, convert_label).load_ndarrays()
    ...
    samples_train=data_generator.flow_from_generators(images_train, labels_train, 20)
    samples_test=data_generator.flow_from_generators(images_test, labels_test, 20)
    first_sample_batch, samples_train=peek(samples_train)
    ...
    batch_size = 20
    epochs = 50
    steps_per_epoch = len(image_names_train) / batch_size
    ...
    history = model.fit_generator(samples_train, steps_per_epoch=steps_per_epoch, epochs=epochs, callbacks=callbacks,
                                  ...
                                  validation_steps=len(image_names_test)/batch_size)
