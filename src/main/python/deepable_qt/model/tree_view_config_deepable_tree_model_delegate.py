...
class TreeViewConfigDeepableTreeModelDelegateFactory(AbstractItemModelDelegateFactory[T]):
...
class TreeViewConfigDeepableTreeModelDelegate(StandardDeepableTreeModelDelegate):
	...
	def data(self, index: T, role: int = Qt.DisplayRole) -> Any:
		model = index.model()
		if index.column() == 0:
	...
	def __has_tree_view_config(self, index: T) -> bool:
		model = index.model()
	...
	def __display_data(self, index: T, view_config: TreeViewConfig) -> Any:
		model = index.model()
		key, obj = model.key(index), model.value(index)
		# obj = deep_get(self.root, key)
		return view_config.display_pattern.format_map(deep_to_dict(obj))

	def __decoration_data(self, index: T, view_config: TreeViewConfig) -> Any:
		model = index.model()
		key, obj = model.key(index), model.value(index)
