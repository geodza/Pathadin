...
class NpzNdarrayLoader(NdarrayLoader):
    ...
    def from_names(file_path: str, names: Iterable[str],
                   ndarray_converter: Callable[[np.ndarray], np.ndarray] = None, mmap_mode=None):
        return NpzNdarrayLoader(file_path, names, ndarray_converter, mmap_mode)
    ...
    def from_name_filter(file_path: str,
                         ...
                         mmap_mode=None):
        names = load_names_from_npz(file_path, name_filter, name_pattern)
        return NpzNdarrayLoader(file_path, names, ndarray_converter, mmap_mode)
    ...
    def load_ndarrays(self) -> Iterable[np.ndarray]:
        return load_ndarrays_from_npz(self.file_path, self.names, self.ndarray_converter, self.mmap_mode)


def load_names_from_npz(file_path: str,
                        name_filter: Callable[[str], bool] = lambda _: True,
...
def load_ndarrays_from_npz(file_path: str,
                           names: Iterable[str],
                           ndarray_converter: Callable[[np.ndarray], np.ndarray] = None,
