if __name__ == '__main__':
	...
	def define_example_original_data():
		...
		for slide_name in slide_names:
			...
			slide_paths.append(
				SlidePath(str(slide_dir.joinpath(slide_name + '.mrxs').resolve()), str(annotations_path.resolve())))
		return slide_paths
	...
	def define_my_custom_data():
		...
		# ]
		slide_paths = [
			SlidePath(r"E:\slides\slide1.mrxs", r"E:\slides\slide1_annotations.json"),
			# SlidePath(r"E:\slides\slide2.mrxs", r"E:\slides\slide2_annotations.json"),
		]
		return slide_paths
	...
	def build_config_for_slide_path(slide_path: SlidePath, level: int, patch_size: int,
									stride_x: int, stride_y: int,
									rescale: bool) -> PatchImageSourceConfig:
		return PatchImageSourceConfig(
			slide_path=slide_path.slide_path,
			level=level,
			target_color_mode='L',
			annotations_path=slide_path.annotations_path,
			metadata={"name": "label", "rescale": rescale, "patch_size": patch_size},
			patch_size=(patch_size, patch_size),
			stride_x=stride_x,
			stride_y=stride_y,
			rescale_result_image=rescale,
			dependents=[
				PatchImageConfig(
					slide_path=slide_path.slide_path,
					level=level,
					target_color_mode='RGB',
					metadata={"name": "image", "rescale": rescale, "patch_size": patch_size},
					rescale_result_image=rescale
				)
			]
		)
	...
	configs = [
		build_config_for_slide_path(slide_path, level,
									patch_size, int(patch_size), int(patch_size),
									rescale_result_image)
		for slide_path in slide_paths
		for level in [1]
		for rescale_result_image in [True, False]
	...
	format_str = r"{cfg.metadata[name]}/{cfg.metadata[name]}/{cfg.slide_path}/{pos[1]},{pos[0]}_{cfg.level}_{cfg.metadata[patch_size]}_rescale_{cfg.metadata[rescale]}_{cfg.metadata[name]}.jpg"
	...
	patches_path = root_path.joinpath("slice_example_param_grid_patches2.zip")
